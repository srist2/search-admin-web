  ...
  <el-container>
    ...
    <el-table
      ...
      <el-table-column type="expand">
        <template slot-scope="props">
          <el-descriptions title="失踪者信息" border :column="4">
            ...
            <el-descriptions-item label="性别">{{props.row.infoSex}}</el-descriptions-item>
            ...
            <el-descriptions-item label="相貌特征及身世描述">{{props.row.infoDescribe}}</el-descriptions-item>
          </el-descriptions>
      ...
      <el-table-column label="联系人姓名">
        <template slot-scope="scope"><p>{{scope.row.contactPerson.ctName}}</p></template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      formatType(type, row) {
        if (type === 'seek') {
          return utils.seekTypeFormat(row);
        }
        if (type === 'miss') {
          return utils.missTypeFormat(row);
        }
      }
    },
    created() {
